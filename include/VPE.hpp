...
using glmvec2 = glm::dvec2 using glmvec3 = glm::dvec3 using glmmat2 = glm::dmat2 using glmmat3 = glm::dmat3 using glmvec4 = glm::dvec4 using glmmat4 = glm::dmat4 using glmquat = glm::dquat const real c_eps = 1.0e-12;
#else
...
// N...normal vector
#define ITORP(X) \
	glmvec3 { contact.m_body_inc.m_to_other *glmvec4{X, 1.0_real} }
#define ITORV(X) glmmat3{contact.m_body_inc.m_to_other} * (X)
#define ITORN(X) contact.m_body_inc.m_to_other_it *(X)
#define ITORTP(X) \
	glmvec3 { face_ref->m_LtoT *contact.m_body_inc.m_to_other *glmvec4{X, 1.0_real} }
#define ITTOWP(X) \
	glmvec3 { contact.m_body_inc.m_body->m_model * face_inc->m_TtoL *glmvec4{X, 1.0_real} }

#define ITOWP(X) \
	glmvec3 { contact.m_body_inc.m_body->m_model *glmvec4{X, 1.0_real} }
#define ITOWN(X) contact.m_body_inc.m_body->m_model_it *(X)

#define RTOIP(X) \
	glmvec3 { contact.m_body_ref.m_to_other *glmvec4{X, 1.0_real} }
#define RTOIN(X) contact.m_body_ref.m_to_other_it *(X)
#define RTOWP(X) \
	glmvec3 { contact.m_body_ref.m_body->m_model *glmvec4{X, 1.0_real} }
#define RTOWN(X) contact.m_body_ref.m_body->m_model_it *(X)
#define RTORTP(X) \
	glmvec3 { face_ref->m_LtoT *glmvec4{X, 1.0_real} }

#define WTOTIP(X) \
	glmvec3 { face_inc->m_LtoT *contact.m_body_inc.m_body->m_model_inv *glmvec4{X, 1.0_real} }
...
#define RTTORP(X) \
	glmvec3 { face_ref->m_TtoL *glmvec4{(X), 1.0_real} }
#define RTTOWP(X) \
	glmvec3 { contact.m_body_ref.m_body->m_model * face_ref->m_TtoL *glmvec4{(X), 1.0_real} }
...
namespace vpe
{
	...
	public:
		...
		/// <returns></returns>
		template <typename T>
		Face *maxFaceAlignment(const glmvec3 dirL, const T &faces, real (*fct)(real) = [](real x)
																   { return x; })
		{
			assert(faces.size() > 0);
			auto compare = [&](Face *a, Face *b)
			{ return fct(glm::dot(dirL, a->m_normalL)) < fct(glm::dot(dirL, b->m_normalL)); };
			return *std::ranges::max_element(faces, compare);
		}

		struct Collider
		{
		}; // Base class for all classes that can collide
		...
		struct Polytope : public Collider
		{
				...
				: Collider{}, m_vertices{}, m_edges{}, m_faces{}, inertiaTensor{inertia_tensor}
			{
				...
				for (auto &edgepair : edgeindices)
				{ // compute edges from indices
				...
				for (auto &face_edge_idx : face_edge_indices)
				{ // compute faces from edge indices belonging to this face
					assert(face_edge_idx.size() >= 2);
					auto &face = m_faces.emplace_back(id++); // add new face to face vector

					for (auto &edge : face_edge_idx)
					{ // add references to the edges belonging to this face
						...
						if (edge.m_factor > 0)
						{
							face.m_face_vertex_ptrs.push_back(&pe->m_first_vertexL);
						} // list of vertices of this face
						else
						{
							face.m_face_vertex_ptrs.push_back(&pe->m_second_vertexL);
						}
					...
					{ // precompute tangent space coordinates of face's vertices
		...
		inline static Polytope g_cube{
			{{-0.5_real, -0.5_real, -0.5_real}, {-0.5_real, -0.5_real, 0.5_real}, {-0.5_real, 0.5_real, 0.5_real}, {-0.5_real, 0.5_real, -0.5_real}, {0.5_real, -0.5_real, 0.5_real}, {0.5_real, -0.5_real, -0.5_real}, {0.5_real, 0.5_real, -0.5_real}, {0.5_real, 0.5_real, 0.5_real}},
		...
		public:
			...
			Body(VPEWorld *physics) : m_physics{physics}
			{
				m_polytope = &m_physics->g_cube;
				inertiaTensorL();
				updateMatrices();
			};
			...
			Body(VPEWorld *physics, std::string name, void *owner, Polytope *polytope,
				 ...
				 real mass_inv = 0, real restitution = 0.2_real, real friction = 1) : m_physics{physics}, m_name{name}, m_owner{owner}, m_polytope{polytope},
			...
			bool stepPosition(double dt, glmvec3 &pos, glmquat &quat, bool use_pbias = true)
			{
				...
				if (m_physics->m_clamp_position == 0)
					pos += m_linear_velocityW * (real)dt;
				...
				{
					if (m_physics->m_clamp_position == 1)
						quat = glm::rotate(quat, len * (real)dt, avL / len); // Euler step
					active = true;
				}
				if (m_physics->m_clamp_position == 0 && len != 0.0_real)
					quat = glm::rotate(quat, len * (real)dt, avL / len); // Euler step
			...
			Vertex *support(glmvec3 dirL)
			{
				...
				{ return glm::dot(dirL, a.m_positionL) < glm::dot(dirL, b.m_positionL); };
		...
		{
			if (contact.m_contact_points.size() == 0)
			{																				   // If first contact point
			...
			else if (d > m_resting_factor * c_gravity * m_sim_delta_time)
			{ // Resting contact
		...
		public:
			...
			{
				return std::find_if(m_vector.begin(), m_vector.end(), [&key](auto pair)
		...
		{
			auto compare = [&](auto &a, auto &b)
			{ return glm::dot(glm::normalize(a.second->m_positionW - pos), dir) < glm::dot(glm::normalize(b.second->m_positionW - pos), dir); };
		...
		void clear()
		{
			for (std::pair<void *, std::shared_ptr<Body>> body : m_bodies)
			{
				auto b = body.second;
				if (b->m_on_erase)
				{					  // if there is a callback for removing the owner
		...
		void addPositionBias(glmvec3 &current_bias, glmvec3 &new_bias)
		{
			...
			if (m_align_position_bias == 1)
			{										// Should we align thenew bias with the old one?
				auto l = glm::length(current_bias); // Length of current bias
				if (l > 0.0_real)
				{																		   // if there is a current bias
		...
		void tick(double dt)
		{
			...
			{									   // if the engine is in realtime mode, advance time
				m_current_time = m_last_time + dt; // advance time by the time that went by since the last loop
				if (dt != 0.0)
					m_fps = 1.0_real / (real)dt; // estimate for frames per second
			}

			auto last_loop = m_loop;
			while (m_current_time > m_next_slot)
			{						  // compute position/vel only at time slots
				...
				for (auto &body : m_bodies)
				{ // integrate positions and update the matrices for the bodies
					if (body.second->stepPosition(m_sim_delta_time, body.second->m_positionW, body.second->m_orientationLW))
						++num_active;
					body.second->updateMatrices();
				}

				m_num_active = 0.9_real * m_num_active + 0.1_real * num_active; // smooth the number of active nodies
				if (m_num_active < c_small)
					m_num_active = 0; // If near 0, set to 0
			...
			if (m_loop > last_loop)
			{ // if we have entered a new time slot bodies might have moved, so update broadphase grid
				for (auto &body : m_bodies)
				{
					moveBodyInGrid(body.second);
				} // update grid
			}
			for (auto &body : m_bodies)
			{ // predict pos/vel at slot + delta, this is only a prediction for rendering, this is not stored anywhere
			...
			for (auto &coll : cell)
			{
				for (auto &neigh : neigh)
				{
					if (coll.second->m_owner != neigh.second->m_owner)
					{
						...
						{
							it->second.m_last_loop = m_loop;
						} // yes - update loop count
						else
						{
		...
		void broadPhase()
		{
			...
			{											   // loop through all cells that are currently not empty.
				makeBodyPairs(m_global_cell, cell.second); // test all bodies against the ground
				for (auto &pi : c_pairs)
				{ // create pairs of neighborig cells and make body pairs.
					intpair_t ni = {cell.first.first + pi.first, cell.first.second + pi.second};
					if (m_grid.count(ni) > 0)
					{
						makeBodyPairs(cell.second, m_grid.at(ni));
					}
				}
			}
		}
		...
		void narrowPhase()
		{
			...
			for (auto it = std::begin(m_contacts); it != std::end(m_contacts);)
			{
				...
				if (contact.m_last_loop == m_loop)
				{ // is contact still possible?
					contact.m_old_contact_points = std::move(contact.m_contact_points);

					if (!warmStartContact(contact))
					{ // Can we completely warm start the contact?
						bool ct = false;
						...
						if (ct)
						{
							if (m_collider.contains(contact.m_body_ref.m_body->m_owner))
								m_collider[contact.m_body_ref.m_body->m_owner](contact.m_body_ref.m_body, contact.m_body_inc.m_body);
							if (m_collider.contains(contact.m_body_inc.m_body->m_owner))
								m_collider[contact.m_body_inc.m_body->m_owner](contact.m_body_inc.m_body, contact.m_body_ref.m_body);
						}
					}
					++it;
				}
		...
		{
			if (m_use_warmstart_single == 0 || contact.m_body_ref.m_body->m_loop_last_active + 2 > m_loop // do not set to 1
				|| contact.m_body_inc.m_body->m_loop_last_active + 2 > m_loop || contact.m_old_contact_points.size() < 3 || contact.m_num_resting != contact.m_old_contact_points.size())
			{ // do not warmstart if non resting contact points present
				return false;
			}
		...
		void warmStart()
		{
			...
			for (auto &c : m_contacts)
			{
				...
				if (num0 < 4 || num1 < 4 || contact.m_contact_points.size() == 0 || contact.m_old_contact_points.size() == 0)
					continue;

				for (auto &cp : contact.m_contact_points)
				{
					if (cp.m_type != Contact::ContactPoint::resting || cp.m_f != 0.0_real)
						continue; // warmstart only once
					for (auto &coldp : contact.m_old_contact_points)
					{
						if (coldp.m_type != Contact::ContactPoint::resting)
							continue; // warmstart only resting points
						if (glm::length(cp.m_positionW - coldp.m_positionW) < c_small)
						{						// if old point is at same position
		...
		{
			if (contact.m_body_inc.m_body->m_positionW.y > contact.m_body_inc.m_body->boundingSphereRadius())
				return false; // early out test
		...
		uint64_t calculateContactPointImpules(Contact &contact)
		{
			...
			for (auto &cp : contact.m_contact_points)
			{
				...
				if (m_solver == 1)
				{ // Separate normal and tangent solver
		...
		SatQuery sat_query(Contact &contact, glmvec3 nR)
		{
			...
			if (minB - maxA > m_collision_margin)
				contact.m_separating_axisW = nW; // Remmber separating axis
		...
		bool SAT(Contact &contact)
		{
			...
			if (fq0.m_separation > m_collision_margin)
			{
				return false;
			}; // found a separating axis with face normal
			...
			if (fq1.m_separation > m_collision_margin)
			{
				return false;
			}; // found a separating axis with face normal
			...
			if (eq.m_separation > m_collision_margin)
			{
				return false;
			} // found a separating axis with edge-edge normal

			contact.m_separating_axisW = glmvec3{0, 0, 0}; // no separating axis found
			if (fq0.m_separation >= eq.m_separation * 1.001_real || fq1.m_separation >= eq.m_separation * 1.001_real)
			{ // max separation is a face-vertex contact
			...
			{
				createEdgeContact(contact, eq);
			} // max separation is an edge-edge contact
			return true;
		}
		...
		FaceQuery queryFaceDirections(Contact &contact)
		{
			...
			auto sat = [&](auto &face) {					   // Run this function for each reference face
				...
				if (sat.m_separation > result.m_separation)
					result = {sat.m_separation, &face, sat.m_vertB}; // remember max distance
		...
		EdgeQuery queryEdgeDirections(Contact &contact)
		{
			...
			for (auto &edgeA : contact.m_body_ref.m_body->m_polytope->m_edges)
			{ // loop over all edge-edge pairs
				for (auto &edgeB : contact.m_body_inc.m_body->m_polytope->m_edges)
				{
					...
					if (sat.m_separation > m_collision_margin)
						return {sat.m_separation, &edgeA, &edgeB}; // if distance positive, stop - we found a separating axis
		...
		real clipFaceFace(Contact &contact, Face *face_ref, Face *face_inc)
		{
			...
			{															  // more than 4 contact points -> reduce to 4
				...
				std::ranges::for_each(dirs0, [&](auto &dirS)
									  { support(dirS, newPolygon, supp); }); // find support points for all 8 directions
				...
				if (A0 > A1)
				{
					newPolygon = std::vector<glmvec2>{supp[0], supp[1], supp[2], supp[3]};
				} // First quadrilateral is bigger
				else
				{
					newPolygon = std::vector<glmvec2>{supp[4], supp[5], supp[6], supp[7]};
				} // Second quadrilateral is bigger
			}
			real min = 0.0_real;
			for (auto &p2D : newPolygon)
			{																	 // Go through all clip points
		...
		public:
			...
			void setUp(real dt) override
			{
				...
				if (m_limit_active)
				{
					...
					if (glm::dot(rel_rot_axis, m_axis1_world) < 0.0_real)
						w = -w; // axes point in different directions
		...
		public:
				  ...
				  m_bodiesNearbyCount{0}
			{
				...
				applyTransformation(glm::rotate( // Apply a slight rotation to give the sim a degree of freedom for all three dimensions
										glm::mat4(1.0f), glm::radians(0.1f), glm::vec3(0.0f, 1.0f, 0.0f)),
									true);
			}
		...
		private:
			...
			void updateBodiesNearby(const std::unordered_map<intpair_t, body_map> &rigidBodyGrid)
			{
				...
				for (const auto &cell : rigidBodyGrid) // Count how many rigid bodies are currently in cell or neighbor cells
					if (std::abs(cell.first.first - gridX) < 2 && std::abs(cell.first.second - gridZ) < 2)
								  ...
								  const std::vector<glmvec3> fixedPointsPositions)
			{
				...
				for (size_t i = 0; i < vertices.size(); ++i)
				{
					...
					else // Create a new mass point if none exists yet
					{
						...
						m_massPoints[m_massPoints.size() - 1].m_associatedVertices.push_back((uint32_t)i);
					}
				}
			}
